...
def testValid(theParam, theValue, defaultValue, startBounds, endBounds, returnDefault = False):
    ...
    try:
        if theParam == "bri" and (theValue.startswith('+') or theValue.startswith('-')):
            # Handle relative brightness changes
            return theValue  # Return as-is for processing later
        
        # Convert to float first to handle decimal values
        theValue = float(theValue)
        
        if theValue < startBounds or theValue > endBounds:
            if not returnDefault:
                if theValue < startBounds:
        ...
        return int(theValue) if theParam != "bri" else theValue  # Return as float for brightness, int for others
    except ValueError:
        printDebugString(f" >> --{theParam} specified is not a valid number - falling back to default value of {defaultValue}")
        return defaultValue
    ...
    if threadAction == "": # if we're not already processing info in another thread
        ...
        if len(paramsList) != 0:
            if paramsList[3] == "discover": # we asked to discover new lights
                ...
                if len(selectedLights) > 0:
                    if paramsList[3] in ["CCT", "HSI", "ANM"]:
                        # Handle relative brightness changes
                        if paramsList[3] in ["CCT", "HSI"] and isinstance(paramsList[5], str) and (paramsList[5].startswith('+') or paramsList[5].startswith('-')):
                            current_brightness = float(availableLights[selectedLights[0]][3][3])  # Assuming brightness is at index 3
                            brightness_change = float(paramsList[5])
                            new_brightness = max(0, min(100, current_brightness + brightness_change))
                            paramsList[5] = new_brightness
